from additional_methods.crowding import crowding
import math
import numpy as np
...
class CMA:
    ...
    def _is_feasible(self, param: np.ndarray) -> bool:
        ...
        return cast(
            bool,
            np.all(param >= self._bounds[:, 0]) and np.all(
    ...
    def tell(self, solutions: List[Tuple[np.ndarray, float]]) -> None:
        ...
        crowded_solution = crowding(solutions, self.previous_population)
        self.previous_population = solutions

        x_k = np.array([s[0] for s in crowded_solution])  # ~ N(m, σ^2 C)
